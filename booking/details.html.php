<!-- This is the HTML form used for USERS to view the DETAILS of their Order information-->
<!DOCTYPE html>
<html lang="en">
	<head>
		<meta charset="utf-8">
		...
		<script>
			function validateUserMessage(){
				...
				if(userMessageSubmitted){
					...
					// No change detected
					var submitConfirmed = confirm("No new message to staff have been detected. Are you sure you want to exit the details overview?");
					return submitConfirmed;
				}
			}
		</script>
			...
			<fieldset><legend>Order Details</legend>
				<div>
					<?php if(isSet($_SESSION['EditBookingOrderError'])) :?>
						...
						<?php unset($_SESSION['EditBookingOrderError']); ?>
					<?php endif; ?>
				</div>
				...
				<div>
					...
					<?php if(empty($originalOrderUserNotes)) : ?>
						<span><b><i><?php htmlout("You did not submit any additional information."); ?></i></b></span>
					<?php else : ?>
				...
				<div>
					<label class="description">Send New Message To Staff: </label>
					<textarea rows="4" cols="50" id="OrderCommunicationToStaff" name="OrderCommunicationToStaff" placeholder="Enter New Message To Staff"><?php htmlout($orderCommunicationToStaff); ?></textarea>
				</div>
				...
				<div class="left">
					...
					<input type="submit" name="details" value="Submit Message" onclick="return validateUserMessage()">
					<input type="submit" name="details" value="Go Back">
					<input type="submit" name="details" value="Reset">
				</div>
			</fieldset>
		</form>
	</body>
